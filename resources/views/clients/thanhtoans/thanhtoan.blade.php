                        ...
                        <div class="checkout-detail-box">
                            <ul>
                                ...
                                <li>
                                            ...
                                            <div class="accordion accordion-flush custom-accordion" id="accordionFlushExample">
                                                @foreach ($phuongThucThanhToan as $item)
                                                    @if ($item->trang_thai == 1)
                                                            ...
                                                            <div class="accordion-header" id="flush-heading{{ $item->id }}">
                                                                <div class="accordion-button collapsed" data-bs-toggle="collapse"
                                                                    ...
                                                                    <div class="custom-form-check form-check mb-0">
                                                                        <label class="form-check-label" for="payment_{{ $item->id }}">
                                                                            <input class="form-check-input mt-0 payment-method" type="radio"
                                                                                name="flexRadioDefault" id="payment_{{ $item->id }}"
                                                        ...
                                                        @if ($item->ten_phuong_thuc == 'VNPAY')

                                                            <div id="flush-collapse{{ $item->id }}"
                                                                class="accordion-collapse collapse vnpay-bank-list"
                                                                ...
                                                                <div class="accordion-body">
                                                                    ...
                                                                    <select id="bank_code" class="form-control">
                                                                        ...
                                                                        <option value="VNBANK">Ngân hàng Việt Nam</option>
                                    ...
                                    <input type="hidden" id="vnpay-id" value="{{ $vnpayId ?? '' }}">
                                    
                                    <!-- Kiểm tra nếu không có phương thức thanh toán -->
                ...
                <div class="col-lg-4">
                    <div class="right-side-summery-box">
                        ...
                        <a href="javascript:void(0);" id="btnDatHang"
                            class="btn theme-bg-color text-white btn-md w-100 mt-4 fw-bold" data-method="VNPAY">
                            Đặt hàng
                        </a>
                    </div>
                </div>
...
@section('js')
    ...
    if (btnDatHang) {
        btnDatHang.addEventListener("click", function (event) {
            event.preventDefault();

            let selectedPaymentMethod = document.querySelector('input[name="flexRadioDefault"]:checked');
            if (!selectedPaymentMethod) {
                alert("Vui lòng chọn phương thức thanh toán!");
                return;
            }

            let paymentMethod = selectedPaymentMethod.dataset.id;
            let totalAmountElement = document.getElementById("tong-tien");
            ...
            let totalAmount = totalAmountElement.innerText.replace(/\D/g, '');
            if (!totalAmount || totalAmount == "0") {
                alert("Giỏ hàng trống, không thể thanh toán!");
                window.location.href = "{{ route('giohang') }}"; // Chuyển hướng về giỏ hàng
                return;
            }

            let vnpayId = document.getElementById("vnpay-id").value.trim();

            // 🔹 Nếu chọn phương thức VNPAY (ID = 2) thì chuyển đến cổng thanh toán
            if (paymentMethod === vnpayId) {
                let bankCodeInput = document.getElementById("bank_code");
                if (!bankCodeInput || !bankCodeInput.value) {
                    alert("Vui lòng chọn ngân hàng trước khi thanh toán!");
                    return;
                }

                let bankCode = bankCodeInput.value;

                let vnpayForm = document.createElement("form");
                vnpayForm.method = "POST";
                vnpayForm.action = "{{ route('vnpay.payment') }}";
                ...
                let inputBankCode = document.createElement("input");
                inputBankCode.type = "hidden";
                inputBankCode.name = "bank_code";
                inputBankCode.value = bankCode;

                vnpayForm.appendChild(csrfInput);
                vnpayForm.appendChild(inputAmount);
                vnpayForm.appendChild(inputBankCode);
                document.body.appendChild(vnpayForm);
                vnpayForm.submit();
            } else {
                // Nếu không phải VNPAY, thì xử lý thanh toán nội bộ
                window.location.href = "{{ route('thanhtoans.dathangthanhcong', ['id' => 1]) }}";
            }
        });
    }

    // 🔹 Ẩn danh sách ngân hàng nếu chọn phương thức khác VNPAY
    let paymentRadios = document.querySelectorAll('.payment-method');
    let vnpayBankLists = document.querySelectorAll('.vnpay-bank-list');

    paymentRadios.forEach(function (radio) {
        radio.addEventListener("change", function () {
            let selectedPaymentId = this.dataset.id;
            if (selectedPaymentId !== vnpayId) {
                vnpayBankLists.forEach(function (list) {
                    let collapseInstance = new bootstrap.Collapse(list, { toggle: false });
